 ...
 * types. Backends translate Claude Agent SDK messages and Codex app-server
 * JSON-RPC notifications into the normalized `SubagentEvent` union.
 */
...
export interface SpawnTask {
   ...
   * claude: model alias; codex: model slug. Omitted = backend default.
   */
...
export interface SubagentMeta {
  ...
  /** OpenCode session log / Claude projects JSONL / Codex rollout path. */
  readonly sessionFilePath?: string;
  /** Claude session id / Codex conversation id. */
  readonly nativeSessionId?: string;
  /** Native child or external-backend shadow OpenCode session. */
