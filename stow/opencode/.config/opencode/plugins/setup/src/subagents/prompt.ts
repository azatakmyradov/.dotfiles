...
export const SUBAGENT_SPAWN_TOOL_DESCRIPTION =
  "Spawn a background external-harness subagent with its own context window on Claude Code or Codex. Fire-and-forget: this returns immediately with an id. The final output is queued back when it settles, or collect it with subagent_wait. Children cannot spawn subagents or ask the user and cannot see this conversation, so prompts must be self-contained. Max 4 subagents can run at once.";

/** Adds background subagent delegation to the parent model's available-tools prompt. */
export const SUBAGENT_SPAWN_PROMPT_SNIPPET =
  "Spawn a background external-harness subagent on Claude Code or Codex for a self-contained task";

/** Guides the parent model to delegate standalone tasks and avoid unnecessary blocking waits. */
export const SUBAGENT_SPAWN_PROMPT_GUIDELINES = [
  "Use subagent_spawn to delegate self-contained tasks that can run in the background; give it a complete, standalone prompt.",
  "Pick Claude Code or Codex deliberately based on the user's request; use OpenCode's native subagent tool for native delegation.",
...
export const SUBAGENT_SPAWN_PARAMETER_DESCRIPTIONS = {
  ...
  harness:
    'External harness: "claude" (Claude Agent SDK) or "codex" (Codex app-server).',
  workingDir: "Working directory (default: current working directory)",
  model:
    "Model hint interpreted by the harness (claude: model alias; codex: model slug). Omit for the harness default.",
  reasoningEffort:
    "Reasoning effort on a shared scale; each harness maps it to its nearest native equivalent.",
};
