 ...
 * backends launch real processes and have their own live test files.
 */

import assert from "node:assert/strict";
import test from "node:test";
import { Effect, Layer, ManagedRuntime } from "effect";
import { BackendRegistry, type SubagentBackend } from "../../src/subagents/backend.ts";

import { makeStubBackend } from "../../src/subagents/backends/stub.ts";
import {
  BACKEND_NAMES,
  type BackendName,
  type ParentContext,
  type SpawnTask,
} from "../../src/subagents/domain.ts";
import {
  SubagentManager,
  SubagentManagerLive,
  type SubagentManagerShape,
} from "../../src/subagents/manager.ts";
import { runTool } from "../../src/subagents/runtime.ts";
...
test("the plugin exposes only external harness backends", () => {
  assert.deepEqual(BACKEND_NAMES, ["claude", "codex"]);
});
